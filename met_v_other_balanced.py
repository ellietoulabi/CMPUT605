...
def main():
    ...
    NaN_method = "zero"
    log_path = (
        f"/home/ellie/Desktop/UAlberta/CMPUT605/Code/CMPUT605/logs/TwoClasses/{rnd}_MVO_balanced_"
        + classifier
        + "_"
        + NaN_method
        + ".log"
    ...
    for subj in subjects:
        ...
        labels  = ['O' if cls == 'LF' or cls == 'SM' or cls=="LT" else cls for cls in mVof]


        X_tmp, y_tmp = load_data(path=path, subject=subj, labels=labels, method=NaN_method)
        ...
        # y = np.array([0 if cls == 'O' else 1 for cls in y_tmp])
        
        indices_M = np.where(y_tmp == 'M')[0]

        indices_O = np.where(y_tmp == 'O')[0]
        ...
        X = X_tmp[selected_indices]
        
        y = np.array([0 if cls == 'O' else 1 for cls in y_tmp[selected_indices]])
    ...
    plt.savefig(
        ...
        + f"{rnd}_MVO_balanced_avg_over_allsubjects_"
        + classifier
        + "_"
        + NaN_method
        + ".jpg"
