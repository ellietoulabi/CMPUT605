...
def generate_custom_cv_folds(sentences, similar_groups, n_splits=5):
    ...
    selection_conditions = [
        {'length_4': 2, 'length_2': 0},  
        {'length_4': 0, 'length_2': 4},  
        {'length_4': 1, 'length_2': 2}   
    ]
...
for i,j,k in zip(sentences,labels,labels_t):
    print(i,j,k)
...
for train_index, test_index in custom_cv_folds:
    ...
    for a, s,t ,d in zip(np.array(sentences)[test_index], np.array(labels)[test_index], np.array(labels_t)[test_index], y_pred):
