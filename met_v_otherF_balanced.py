...
def main():
    ...
    NaN_method = "zero"
    log_path = (
        f"/home/ellie/Desktop/UAlberta/CMPUT605/Code/CMPUT605/logs/TwoClasses/{rand}_MVOF_"
        + classifier
        + "_"
        + NaN_method
        + ".log"
    ...
    for subj in subjects:
        ...
        X_t =   np.array([X_tmp[i] for i in indices if i < len(X_tmp)]  )
        y_t = np.array([0 if cls == 'F' else 1 for cls in y_c])
        
        indices_M = np.where(y_t == 1)[0]

        indices_O = np.where(y_t == 0)[0]
        ...
        X = X_t[selected_indices]
        
        y = y_t[selected_indices]
    ...
    plt.savefig(
        ...
        + f"{rand}_MVOF_avg_over_allsubjects_"
        + classifier
        + "_"
        + NaN_method
        + ".jpg"
