...
sentences = list(map(lambda x : x.replace('_', ' ').replace('Some', '').replace('are', ''), pd.read_excel('./stimNamesAndTypeforGONZO.xlsx')['stim'].values) )
...
for subj in subjects:
    ...
    for train_index, test_index in tqdm(loo.split(X)):
        ...
        corr = corr_score_row(y_test, y_pred)[0]
    ...
    logging.info(f"\n-----Subject Results:\n")
    logging.info(f"\nAverage MSE:\n{np.average(outer_cv_results)}")
    logging.info(f"\nAverage Corr:\n{np.average(outer_corrs)}")
    ...
    print(f"p-value:{np.sum(np.array(permuted_corrs) >= observeed_corr) / num_permutations}")
